...
export const attemptPendingPayments = async (bot: Telegraf) => {
  const pendingPayments = await PendingPaymentModel.find({
    paid: false,
  ...
  for (const pending of pendingPayments) {
    const order = await OrderModel.findOne({ _id: pending.order_id });
    try {
      pending.attempts++;
      ...
      const buyerUser = await UserModel.findOne({ _id: order.buyer_id });
      ...
      if (!!payment && !!payment.confirmed_at) {
        ...
        const sellerUser = await UserModel.findOne({ _id: order.seller_id });
        ...
        if (
          pending.attempts ===
          parseInt(process.env.PAYMENT_ATTEMPTS as string, 10)
        ) {
          order.paid_hold_buyer_invoice_updated = false;
...
export const attemptCommunitiesPendingPayments = async (bot: Telegraf) => {
  ...
  for (const pending of pendingPayments) {
    try {
      ...
      const user = await UserModel.findById(pending.user_id);
      ...
      const community = await CommunityModel.findById(pending.community_id);
      if (!!payment && !!payment.confirmed_at) {
        pending.paid = true;
        pending.paid_at = new Date().toISOString();
        ...
        if (
          pending.attempts === parseInt(process.env.PAYMENT_ATTEMPTS as string)
        ) {
          await bot.telegram.sendMessage(
            user.tg_id,
